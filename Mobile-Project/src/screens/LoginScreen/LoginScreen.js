...
import  {apiGabriel, loginResource } from "../../Services/Service.js";
...
const LoginScreen = ({ navigation }) => {
  ...
  const [user, setUser] = useState({ email: "caio@caio.com", senha: "12345" });
  // const { userData, setUserData } = useContext(AuthContext);

  const Login = async () => {
    setLoading(loading); //ao ficar como true, indica que o spinner de loading do botão deve aparecer
    try {
      const response = await apiGabriel.post(loginResource, user);
